...
function getReviewStorageErrorResponse() {
  return NextResponse.json(
    {
      error: "Review storage is not installed yet. Apply the updated Supabase schema so reviews can be edited.",
    },
    { status: 501 },
  );
}
...
async function buildReviewPayload(input: ParsedAdminReview) {
  ...
  return {
    ...
    payload: {
      ...
      rating: input.rating,
      status: input.status,
      is_featured: input.status === "approved" ? input.isFeatured : false,
...
export async function PATCH(request: Request, { params }: Props) {
  try {
    ...
    const userRateLimit = await applyRateLimit({
      key: `admin:reviews:update:user:${user.id}`,
    ...
    return NextResponse.json({ error: getErrorMessage(error, "Unable to update the review right now.") }, { status: 500 });
  }
}
