...
export async function POST(request: Request) {
  try {
    ...
    const image = await verifyRasterImageUpload({ bytes, declaredType: file.type, label: "product media" });
    const objectPath = `products/${productId}/${slot}-${Date.now()}-${crypto.randomUUID()}${image.extension}`;
