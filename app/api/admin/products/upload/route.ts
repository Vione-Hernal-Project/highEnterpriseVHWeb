...
export async function POST(request: Request) {
  try {
    ...
    const slot = sanitizeStoragePathSegment(String(formData.get("slot") || "media")) || "media";
    ...
    const objectPath = `products/${productId}/${slot}-${Date.now()}-${crypto.randomUUID()}${verifiedUpload.extension}`;
