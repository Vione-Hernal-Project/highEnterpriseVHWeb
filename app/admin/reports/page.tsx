...
const RECENT_REPORTS_PREVIEW_LIMIT = 5;
...
function RecentReportsTable({ report, selectedTab }: { report: AdminReportData; selectedTab: ReportTabKey }) {
  ...
  const safeRows = (rows.length ? rows : report.recentReports).slice(0, RECENT_REPORTS_PREVIEW_LIMIT);
  ...
  return (
    <section className="vh-admin-panel vh-admin-analytics-panel vh-admin-report-recent-panel">
      <div className="vh-admin-panel__header">
        ...
        <Link href={`/admin/reports/history?range=${encodeURIComponent(report.range.key)}`}>View All</Link>
      </div>
      <AdminRecentReportsTable rows={safeRows} rangeKey={report.range.key} />
    </section>
  );
}
